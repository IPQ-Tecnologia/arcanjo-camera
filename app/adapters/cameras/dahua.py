...
def extrair_multipart(
    ...
    for parte in body.split(delimitador):
        ...
        cabecalho, conteudo = parte.split(
            b"\r\n\r\n",
            1,
        )
        ...
        if (
            b"application/json" in cabecalho_lower
            or b"text/plain" in cabecalho_lower
        ):
            try:
    ...
    if payload_json is None:
        raise ValueError(
            "Metadados JSON não encontrados no multipart Dahua"
        )
...
def detectar_escala_normalizada(
    ...
    if coordenadas_regiao:
        return max(coordenadas_regiao) > max(
            largura_img,
            altura_img,
        )
...
def converter_bbox_para_pixels(
    ...
    elif detectar_escala_normalizada(
        dados,
        bbox,
        largura_img,
        altura_img,
    ...
    x1_int = max(
        0,
        min(largura_img - 1, round(x1)),
    )
    y1_int = max(
        0,
        min(altura_img - 1, round(y1)),
    )
    x2_int = max(
        0,
        min(largura_img - 1, round(x2)),
    )
    y2_int = max(
        0,
        min(altura_img - 1, round(y2)),
    )
...
def salvar_imagens(
    ...
    caminho_original = (
        PASTA_IMAGENS
        / f"{nome_base}_original.jpg"
    )
    caminho_marcada = (
        PASTA_IMAGENS
        / f"{nome_base}_marcada.jpg"
    )
    ...
    with Image.open(io.BytesIO(imagem)) as imagem_pil:
        ...
        if bbox is not None:
            ...
            espessura = max(
                2,
                min(largura_img, altura_img) // 300,
            )
...
def _normalizar_coordenada_dahua(
    valor,
) -> float | None:
    numero = _numero_attributes_dahua(
        valor
    )
    ...
    # Os pontos de região observados na Dahua
    # usam escala de 0 a 8191.
    if numero > 1:
        numero = numero / 8191
...
def _normalizar_target_type_dahua(
...
def _normalizar_event_type_dahua(
    tipo_evento: str,
...
def _montar_atributos_dahua(
    ...
    geometria = _extrair_geometria_dahua(
        dados.get("DetectRegion")
        or dados.get("Region")
        or dados.get("Polygon")
    )
    ...
    acao = (
        dados.get("Action")
        or objeto.get("Action")
        or acao_original
    )

    direcao = (
        dados.get("Direction")
        or objeto.get("Direction")
        or objeto.get(
    ...
    confidence = (
        _numero_attributes_dahua(
            objeto.get("Confidence")
        )
    )

    # Nas capturas analisadas, zero significa
    # que a confiança não foi disponibilizada.
    ...
    return EventAttributes(
        ...
        category=_categoria_evento_dahua(
            tipo_evento
        ),
        target_type=alvo_detectado,
        state=estado,
        action=_texto_attributes_dahua(
            acao
        ),
        source_event_id=(
            _texto_attributes_dahua(
        ...
        rule_id=_texto_attributes_dahua(
            dados.get("RuleID")
        ),
        rule_name=_texto_attributes_dahua(
            dados.get("Name")
        ),
        group_id=_texto_attributes_dahua(
            dados.get("GroupID")
        ),
        object_id=_texto_attributes_dahua(
            objeto.get("ObjectID")
        ),
        sensitivity=None,
        ...
        geometry=geometria,
        raw_bounding_box=(
            [float(valor) for valor in bbox_bruta]
...
class DahuaAdapter(CameraAdapter):
    ...
    def normalizar(
        ...
        if "multipart/x-mixed-replace" in tipo_conteudo:
            ...
            payload, imagem = extrair_multipart(
                content_type,
                body,
            )
        else:
        ...
        alvo_normalizado = (
            _normalizar_target_type_dahua(
                alvo_detectado
            )
        )

        tipo_evento_normalizado = (
            _normalizar_event_type_dahua(
                tipo_evento,
                alvo_normalizado,
            )
        )
