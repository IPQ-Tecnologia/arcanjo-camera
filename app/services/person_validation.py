...
def calcular_metricas_sobreposicao(
    caixa_a: Any,
    caixa_b: Any,
) -> tuple[float, float]:
    """
    ...
    ax1, ay1, ax2, ay2 = _obter_limites(
        caixa_a
    )

    bx1, by1, bx2, by2 = _obter_limites(
        caixa_b
    )
...
def validar_boxes_camera_com_yolo(
    ...
    """
    Mantém somente as caixas da câmera que possuem
    correspondência espacial com uma pessoa detectada
    pelo YOLO.

    A caixa original da câmera é preservada para que
    o restante do pipeline continue funcionando sem
    alterações.
    """
    ...
    for caixa_camera in caixas_camera:
        ...
        for caixa_yolo in caixas_yolo:
            ...
            if (
                iou >= iou_minimo
                or cobertura >= cobertura_minima
            ):
                caixa_confirmada = True
                break

        if caixa_confirmada:
            caixas_validadas.append(
                caixa_camera
            )
