...
def _filtrar_caixas_validas(
    ...
    """
    Remove caixas inválidas, muito pequenas ou que
    representam praticamente a imagem inteira.
    """
...
def _montar_descricao(
    ...
    if pares_muito_proximos:
        partes.append(
            f"{pares_muito_proximos} par(es) "
            "muito próximo(s)."
        )

    if pares_proximos:
        partes.append(
            f"{pares_proximos} par(es) "
            "próximo(s)."
        )
