...
def processar_evento(body: bytes, content_type: str, evento_id: str):
    ...
    try:
        tipo_conteudo = (content_type or "").lower()
        xml_direto = (
        ...
        if xml_direto:
            ...
            if inicio_xml == -1 or fim_xml == -1:
                print(f"[{evento_id}] Corpo XML inválido")
                return

            xml = xml[inicio_xml:fim_xml + 1]
            print(f"[{evento_id}] XML direto recebido; pacote sem imagem")
            ...
            if not boundary:
                print(f"[{evento_id}] Boundary não encontrado")
                return
            ...
            if xml is None:
                print(f"[{evento_id}] XML não encontrado no multipart")
                return

            if imagem is None:
                print(f"[{evento_id}] Imagem não encontrada no multipart")
        ...
        largura_img = altura_img = None
        boxes_resultado = boxes_xml
        box_escolhido = None
        ...
        if imagem is not None:
            ...
            boxes_resultado = converter_boxes_para_pixels(
                boxes_xml,
                raiz,
                largura_img,
                altura_img,
            )
            box_escolhido = escolher_bounding_box(boxes_resultado)
        ...
        dados_evento = {
            ...
            "dimensoes_imagem": (
                {"largura": largura_img, "altura": altura_img}
                if imagem is not None
                else None
            ),
            "quantidade_bounding_boxes": len(boxes_resultado),
            "bounding_boxes_encontradas": boxes_resultado,
        ...
        if imagem is None:
            dados_evento["observacao"] = (
                "XML processado sem imagem; nenhuma imagem foi salva ou marcada."
            )
        ...
        print(f"[{evento_id}] XML inválido após {tempo_erro_ms} ms: {erro}")
